...
public class Bewegung {
    ...
    public void steuerung() {
        ...
        if (playerY >= GameSetings.getMain_Frame_Height() - player.getPlayerheight()) {
            playerY = GameSetings.getMain_Frame_Height() - player.getPlayerheight();
            playerVY = 0;
            // Level geschafft
        }
        // Fenster Rechts durchgang nach Links
        if (playerX >= GameSetings.getMain_Frame_Width() + player.getPlayerwidth() / 2) {
            playerX = -player.getPlayerwidth();
        }
        // Fenster Links durchgang nach Rechts
        if (playerX < -player.getPlayerwidth()) {
            playerX = GameSetings.getMain_Frame_Width();
        }
